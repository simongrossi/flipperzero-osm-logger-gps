        ...
        while(furi_hal_serial_async_rx_available(app->serial)) {
            ...
            if(c == '\n' || c == '\r') {
                if(app->nmea_pos > 0) {
                    ...
                    bool has_fix = false;
                    float lat = 0, lon = 0, hdop = 99.9f;
                    uint8_t sats = 0;

                    nmea_parse_line(app->nmea_line, &has_fix, &lat, &lon, &hdop, &sats);
...
int32_t app(void* p) {
    ...
    submenu_add_item(app->menu, "Mode rapide (Quick Log)", AppViewQuickLog, app_menu_callback, app);
    submenu_add_item(app->menu, "Statut GPS (vibre/LED + log)", 2, app_menu_callback, app);
